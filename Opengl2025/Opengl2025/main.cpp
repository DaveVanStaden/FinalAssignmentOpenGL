...
//Program IDs
GLuint simpleProgram, skyProgram, terrainProgram, modelProgram, toneMappingShader;
GLuint hdrFBO, colorBuffer, rboDepth;
GLuint quadVAO, quadVBO; // For rendering the quad in tone mapping
...
int main() {
	...
	//Initializing shaders and geometry
	createShaders();
...
void createShaders() {
	...
	createProgram(toneMappingShader, "shaders/tonemapVertex.shader", "shaders/tonemapFragment.shader");
...
void renderToneMap() {
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, colorBuffer); // colorBuffer from HDR FBO
	glUniform1i(glGetUniformLocation(toneMappingShader, "hdrBuffer"), 0);

	// --- Add these lines here ---
	float sunDot = glm::dot(glm::normalize(lightPosition), glm::vec3(0, 1, 0));
	float sunIntensity = glm::clamp(sunDot, 0.0f, 1.0f);
	float minExposure = 0.5f;
	float maxExposure = 5.0f;
	float exposure = minExposure + (maxExposure - minExposure) * pow(sunIntensity, 0.5f);
	// Set exposure uniform based on sun intensity
